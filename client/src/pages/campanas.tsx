...
export default function Campanas() {
  ...
  const [tabActiva, setTabActiva] = useState("canvas");
  const [nuevaCampana, setNuevaCampana] = useState({
    nombre: "",
    secuencia: [] as PasoSecuencia[],
    touchpoints: [] as TouchpointNode[],
    segmentacion: {
      mesesSinVisita: 12,
  ...
  const handleIniciarCampana = () => {
    ...
    // Usar touchpoints del canvas si están disponibles, sino usar secuencia tradicional
    const touchpoints = nuevaCampana.touchpoints.length > 0 
      ? nuevaCampana.touchpoints 
      : nuevaCampana.secuencia.map((p, idx) => ({
          id: p.id,
          canal: p.canal,
          x: idx * 200 + 100,
          y: 200,
          mensaje: p.mensaje,
          diasEspera: p.diasEspera,
        }));
        ...
        <Tabs value={tabPrincipal} onValueChange={setTabPrincipal} className="w-full">
          ...
          <TabsContent value="reactivacion" className="mt-6 space-y-6">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <Button 
                onClick={() => setLocation("/campanas/nueva")}
                data-testid="button-nueva-campana"
              >
